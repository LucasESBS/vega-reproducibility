...
def train_sparse_vae():
    ...
    random_seed = 1
    # Out dir
    local_out = '/trained_models/compare_models/'
    ...
    # Initialize CV
    kfold = KFoldTorch(cv=10, n_epochs=N_EPOCHS, lr=LR, train_p=25, test_p=25, num_workers=0, save_all=True, save_best=False, path_dir=dir_name, model_prefix='sparse_vae_')
    dict_params = {'pathway_mask':pathway_mask, 'beta':0.00005, 'dropout':p_drop, 'path_model':None, 'device':dev, 'positive_encoder':False, 'positive_decoder':True}
    kfold.train_kfold(PathwaySparseVAE, dict_params, train_ds, batch_size=64)
    np.save(dir_name+'10CV_sparse_vae.npy', kfold.cv_res_dict)
    return
