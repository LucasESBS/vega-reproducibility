...
def train_deep_decoder():
    ...
    # Initialize CV
    kfold = KFoldTorch(cv=10, n_epochs=N_EPOCHS, lr=LR, train_p=25, test_p=25, num_workers=0, save_all=True, save_best=False, path_dir=dir_name, model_prefix='deep_decoder_')
    dict_params = {'pathway_mask':pathway_mask, 'beta':0.00005, 'dropout':p_drop, 'path_model':None, 'device':dev, 'positive_encoder':True}
    kfold.train_kfold(PathwayDeepDecoderVAE, dict_params, train_ds, batch_size=64)
    np.save(dir_name+'10CV_deep_decoder.npy', kfold.cv_res_dict)
    return
