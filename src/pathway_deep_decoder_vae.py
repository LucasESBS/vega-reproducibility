...
class PathwayDeepDecoderVAE(torch.nn.Module):
    def __init__(self, pathway_mask, positive_encoder=False, **kwargs):
        """ Constructor for class VAE with sparse encoder, deep decoder"""
        super(PathwayDeepDecoderVAE, self).__init__()
        ...
        print(self.dropout)
        self.mean = nn.Sequential(CustomizedLinear(self.pathway_mask),
                                  nn.Dropout(self.dropout))
        self.logvar = nn.Sequential(CustomizedLinear(self.pathway_mask),
                                    nn.Dropout(self.dropout))
        self.decoder = nn.Sequential(nn.Linear(self.n_pathways, 800),
                                    nn.BatchNorm1d(800),
                                    nn.ReLU(),
                                    nn.Dropout(self.dropout),
                                    nn.Linear(800, 800),
        ...
        if self.pos_enc:
            ...
            self.mean[0].reset_params_pos()
            self.mean[0].weight.data *= self.mean[0].mask
            self.logvar[0].reset_params_pos()
            self.logvar[0].weight.data *= self.logvar[0].mask   
    ...
    def train_model(self, train_loader, learning_rate, n_epochs, train_patience, test_patience, test_loader=False, save_model=True):
        ...
        for epoch in range(n_epochs):
            ...
            for x_train in train_loader:
                ...
                optimizer.step()
                if self.pos_enc:
                    self.mean[0].apply(clipper)
                    self.logvar[0].apply(clipper)
            # Get epoch loss
